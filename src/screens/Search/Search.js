  ...
  useEffect(async () => {
    ...
    const unsub = onSnapshot(q, (snapshot) => {
      snapshot.docs.map((doc) => {
        try {
            if (doc.data().name.toLowerCase().includes(" "+itemSearch.toLowerCase()+" ")){
    
                data.push(doc.data())
    
            } else if(doc.data().name.toLowerCase().includes(itemSearch.toLowerCase())){
                data.push(doc.data())
            }
        } catch (error) {
                ...
                <div className="container-ff">
                    <div className="mainf-2">
                        {itemSearch != '' ?
                            product
                            // .filter((data) => {
                            //     if (data.name.contains(itemSearch) ) {
                            //         return data
                            //     }
                                    ...
                                    {data.quantity == 0 ?
                                        ...
                                        <>
                                            <Link to={`/ProductDetail/${data.name}`} style={{ textDecoration: 'inherit', color: 'inherit' }}>
                                                ...
                                                />
                                                {data.discountPrice?
                                                    <div style={{ backgroundColor: '#fb550e', position: 'relative', bottom: '100px', left: '5px', border: "1px solid #FFA500", borderRadius: "50%", width: '35%' }}>
                            ...
                            :
                                    <h1 style={{ textAlign:'center' }}>Please Search The Product</h1>
                            }
                        {
                         productF
                            .filter((data) => {
...
<Typography variant="caption" color="text.secondary">
    ...
    return  <h1 style={{ textAlign:'center' }}>Sorry! Product Not Found</h1>
